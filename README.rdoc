...
More detail {Meet Jenkins}[http://wiki.jenkins-ci.org/display/JENKINS/Meet+Jenkins]

== Requirements
* Require Redmine 2.1.0 higher.
* Recommend Jenkins.
...
You can change limit settings on settings/plugin/redmine_jenkins.
...
You can change 'AutoFetch' setting on settings/plugin/redmine_jenkins.
...
* rename Gemfile_for_devtest to Gemfile
* RAILS_ENV=test rake redmine_jenkins:db:test:prepare
* RAILS_ENV=test rake redmine_jenkins:cucumber


== Changelog (redmine_hudson)
