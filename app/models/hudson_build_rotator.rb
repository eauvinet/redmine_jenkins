...
def HudsonBuildRotator.can_store?(job, number)
  ...
  # get oldest data
  oldest = HudsonBuild.where(["#{HudsonBuild.table_name}.hudson_job_id = ? and #{HudsonBuild.table_name}.id not in \
                              (select #{HudsonBuild.table_name}.id from #{HudsonBuild.table_name} where #{cond})", job.id]).
...
def HudsonBuildRotator.create_cond_num_to_delete(job_id, num_to_keep)
  ...
  build_count = HudsonBuild.where(["#{HudsonBuild.table_name}.hudson_job_id = ?", job_id]).count
  delete_count = build_count - num_to_keep

  return "" unless delete_count > 0

  delete_builds = HudsonBuild.where(["#{HudsonBuild.table_name}.hudson_job_id = ?", job_id]).
                              order("#{HudsonBuild.table_name}.number ASC").
                              limit(delete_count).offset(delete_count - 1)
