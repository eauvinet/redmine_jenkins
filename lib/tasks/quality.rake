desc 'Auto Code Review Redmine Jenkins Plugin'
begin
  namespace :redmine_jenkins do
    task :reek => [:cd_plugin_dir, :environment] do
      desc 'check code smell(reek) for Jenkins Plugin'
      require "reek"
    ...
    task :flog => [:cd_plugin_dir, :environment] do
      desc 'check complexity for Jenkins Plugin'
      require "flog"
      system "flog app lib test > flog.log"
    end

    task :flay => [:cd_plugin_dir, :environment] do
      desc 'check dry for Jenkins Plugin'
      require "flay"
